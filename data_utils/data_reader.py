...
keyed with the following values:

    raw_data[team_id][year][week]
"""
def parse_all_stats(directory, gids, team_ids):
    ...
    for filename in data_files:
        ...
        for row in reader:
            ...
            team_id = team_ids[row["TeamName"]]

            if team_id not in raw_data.keys():
                raw_data[team_id] = dict()
            if year not in raw_data[team_id].keys():
                raw_data[team_id][year] = dict()

            raw_data[team_id][year][str(nfl_week)] = row
            raw_data[team_id][year][str(nfl_week)]["year"] = str(year)
            raw_data[team_id][year][str(nfl_week)]["week"] = str(nfl_week)
